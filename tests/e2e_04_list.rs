...
fn list_multiple_filters() {
    let dir = setup();
    todos_cmd(dir.path())
        .args(["add", "A", "-p", "high", "-l", "bug"])
        .assert()
        .success();
    todos_cmd(dir.path())
        .args(["add", "B", "-p", "low", "-l", "bug"])
        .assert()
        .success();
    todos_cmd(dir.path())
        .args(["add", "C", "-p", "high", "-l", "feature"])
        .assert()
        .success();
...
fn list_flat_display() {
    ...
    let pid = parent["data"]["task"]["id"].as_str().unwrap();
    todos_cmd(dir.path())
        .args(["add", "子", "--parent", &pid[..8], "-p", "high"])
        .assert()
        .success();
    // --flat でソート順に従う
