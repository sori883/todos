...
// ❌ 悪い: 直接的な状態参照
setCount(count + 1)  // 非同期シナリオで古い値になりうる
```
 ...
 * @throws {Error} OpenAI APIの失敗またはRedis利用不可の場合
 *
 * @example
 * ```typescript
...
```typescript
// ✅ 良い: 説明的なテスト名
test('クエリにマッチするマーケットがない場合に空配列を返す', () => { })
test('OpenAI APIキーが未設定の場合にエラーをスローする', () => { })
test('Redis利用不可時にサブストリング検索にフォールバックする', () => { })
...
**注意**: コード品質は交渉の余地がない。明確でメンテナンス可能なコードが、迅速な開発と自信を持ったリファクタリングを可能にする。
